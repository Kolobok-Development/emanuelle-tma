...
console.log(
  "🚀 Starting Combined Queue Workers (AI Response + Image Generation)..."
);
console.log(
  ...
  async (job: Job<AIResponseJobData>) => {
    ...
    try {
      ...
      if (dbChatId) {
        try {
          conversationHistory =
            await ConversationService.getTokenAwareConversationHistory(
              dbChatId
            );
        } catch (contextError) {
          console.error(
      ...
      if (responseMessage.length > TELEGRAM_MAX_MESSAGE_LENGTH) {
        console.warn(
          `Response message too long (${responseMessage.length} chars), truncating to ${TELEGRAM_MAX_MESSAGE_LENGTH}`
        );
        responseMessage =
          responseMessage.substring(0, TELEGRAM_MAX_MESSAGE_LENGTH - 10) +
          "...";
      }
      ...
      if (dbChatId && responseMessage) {
        try {
          ...
          console.error(
            "Failed to save AI response to conversation history:",
            saveError
          );
        }
      }
      ...
      console.error(
        `Error processing AI response job for chat ${chatId}:`,
        error
      );
...
aiResponseWorker.on("failed", async (job, err) => {
  if (job) {
    ...
    if (job.attemptsMade >= (job.opts.attempts || 3)) {
      console.log(
        `Moving job ${job.id} to dead letter queue after ${job.attemptsMade} attempts`
      );
      try {
...
const imageGenerationWorker = new Worker(
  ...
  async (job: Job<ImageGenerationJobData>) => {
    ...
    try {
      ...
      console.error(
        `Error processing image generation job for chat ${chatId}:`,
        error
      );
...
imageGenerationWorker.on("failed", async (job, err) => {
  if (job) {
    ...
    if (job.attemptsMade >= (job.opts.attempts || 3)) {
      console.log(
        `Moving image generation job ${job.id} to dead letter queue after ${job.attemptsMade} attempts`
      );
      try {
        ...
        console.error(
          `Failed to move image generation job ${job.id} to DLQ:`,
          dlqError
        );
      }
    }
...
console.log(
  "   - Image Generation Worker: processing image generation requests"
);
