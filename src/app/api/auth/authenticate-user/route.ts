...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { initData, authContext = "hub", companionId: bodyCompanionId } = body;

    if (!initData) {
      globalThis?.logger?.warn({}, "No initData provided");
      response = NextResponse.json({ error: "No initData provided" }, { status: 400 });
      return response;
    }

    let resolvedScope: AppScope = AppScope.hub;
    let lockedCompanionId: string | undefined;
    let botToken: string | undefined;

    if (authContext === "dedicated") {
      if (!bodyCompanionId) {
        globalThis?.logger?.warn({}, "dedicated auth requires companionId");
        response = NextResponse.json(
          { error: "companionId required for dedicated auth" },
          { status: 400 }
        );
        return response;
      }
      const botRow = await prisma.companionTelegramBot.findUnique({
        where: { companion_id: bodyCompanionId },
      });
      if (!botRow) {
        globalThis?.logger?.warn({ companionId: bodyCompanionId }, "No Telegram bot for companion");
        response = NextResponse.json(
          { error: "Invalid companion or bot not configured" },
          { status: 401 }
        );
        return response;
      }
      botToken = botRow.bot_token;
      resolvedScope = AppScope.dedicated;
      lockedCompanionId = bodyCompanionId;
    } else {
      botToken = getMainBotToken();
      resolvedScope = AppScope.hub;
      lockedCompanionId = undefined;
    }

    const devBypass = process.env.NODE_ENV === "development";
    const isAuthorized =
      devBypass || (botToken !== undefined && isValid(initData, botToken));
    ...
    globalThis?.logger?.info(
      { telegramUserId: telegramUser.id, username: telegramUser.username, authContext },
    ...
    const jwtPayload: Record<string, unknown> = {
      ...
      app_scope: resolvedScope,
    };
    if (lockedCompanionId) {
      jwtPayload.locked_companion_id = lockedCompanionId;
    }

    const token = await encrypt(jwtPayload, expiresAt);
    ...
    await prisma.session.create({
      data: {
        ...
        app_scope: resolvedScope,
        locked_companion_id: lockedCompanionId ?? null,
      },
    });
